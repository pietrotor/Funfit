...
export function UserProducts () {
  ...
  return (
    <div>
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 gap-y-5 py-8 w-full">
        {products.map((product) => (
            <UserCard key={product.id} name={product.name} description={product.description} price={product.price} image={product.image} images={product.images} />
        ))}
      </div>
    </div>
  )
